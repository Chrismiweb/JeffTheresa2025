...
//         <h2 className="font-script text-white text-2xl sm:text-3xl lg:text-5xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.35)]">
...
function Block({ value, label }) {
  const v = label === "DAYS" ? String(value) : String(value).padStart(2, "0");
  return (
    <div className="text-center w-[84px] sm:w-[104px] md:w-[124px]">
      <div
        className="tabular-nums select-none font-bold text-white leading-none
                   text-[42px] sm:text-[56px] md:text-[72px] lg:text-[84px]
                   drop-shadow-[0_2px_10px_rgba(0,0,0,0.35)]"
      >
        {v}
      </div>
      <div className="mt-3 tracking-[0.35em] text-white/85 text-[10px] sm:text-xs md:text-sm">
        {label}
      </div>
    </div>
  );
  ...
  return (
    <section className="relative min-h-[55vh] grid place-items-center overflow-hidden font-serif">
      ...
      <div className="relative z-10 text-center px-4 max-w-5xl w-full">
        {/* Heading constrained so it wraps nicely on small screens */}
        <h2
          className="font-script text-white leading-tight mx-auto
        ...
        {!weddingIsToday && (
          <div
            className="mt-6 sm:mt-8
                       ...
                       gap-y-6 gap-x-4 sm:gap-x-8 lg:gap-x-14"
            aria-live="polite"
          >
            <Block value={t.days} label="DAYS" />
            <Block value={t.hours} label="HOURS" />
